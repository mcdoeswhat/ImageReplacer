...
public class ImageUtil {
    public static void main(String[] args) throws Exception {
        ...
        createImage(message, 350, image_height, every_line, line_height);

    }

    public static List<BufferedImage> createImage(String text, int width, int image_height, int every_line, int line_height) throws Exception {
        List<BufferedImage> images = new ArrayList<>();
        FontMetrics fm = sun.font.FontDesignMetrics.getMetrics(new Font("微软雅黑", Font.BOLD, 22));
        ...
        int a = newList.size();
        int imgNum = a % every_line == 0 ? (a / every_line) : (a / every_line) + 1;
        ...
        for (int m = 0; m < imgNum; m++) {
            ...
            g.setColor(Color.BLACK); // 背景色白色
            g.fillRect(0, 0, width, image_height);
            g.setColor(Color.WHITE);//  字体颜色黑色
            g.setFont(new Font("宋体", Font.PLAIN, 22));// 设置画笔字体
